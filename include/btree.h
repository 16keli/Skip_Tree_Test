...
public:
    ...
    /// Tries to locate a key in the B+ tree and returns a constant iterator
    /// to the key/data slot if found. If unsuccessful returns end().
    const_iterator get(int index) {
        int total;
        leaf_node* l;
        int slot;
        if (index > size() / 2) {
            l = m_tailleaf;
            total = size() - 1;
            while (total - l->slotuse > index) {
                total -= l->slotuse;
                l = l->prevleaf;
            }
            slot = l->slotuse - (total - index);
        } else {
            l = m_headleaf;
            total = 0;
            while (total + l->slotuse < index) {
